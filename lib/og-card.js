...
export function cardElement(planet, { origin = 'https://go-astray.vercel.app' } = {}) {
  ...
  const sealed = !!(planet && planet.sealed);
  const voice = !!(planet && planet.voiceUrl);
  const kicker = sealed ? 'sealed · made for someone' : (message || song || voice) ? 'someone made this for you' : 'a planet in ASTRAY';
  const sealLine = sealed ? `opens on ${new Date(planet.revealAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', timeZone: 'UTC' })}` : null;
  ...
  return h('div', {
    ...
    h('div', { display: 'flex', flexDirection: 'column', justifyContent: 'center', width: 730, height: OG_HEIGHT, paddingRight: 72, paddingLeft: 6 }, [
      ...
      sealed
        ? h('div', { fontFamily: 'Serif, serif', fontStyle: 'italic', fontSize: 36, color: 'rgba(244,241,232,0.92)', marginTop: 22, display: 'flex' }, sealLine)
        : message
          ...
          : h('div', { fontFamily: 'Serif, serif', fontStyle: 'italic', fontSize: 30, color: INK_DIM, marginTop: 22, display: 'flex' }, voice ? 'they left you their voice' : 'tap to fly there'),
      !sealed && voice && !song
        ? h('div', { display: 'flex', alignItems: 'center', fontFamily: 'Mono, monospace', fontSize: 17, letterSpacing: 2, color: INK_DIM, marginTop: 26, textTransform: 'uppercase' }, [
          h('div', { width: 10, height: 16, borderRadius: 5, border: `2px solid ${AMBER}`, marginRight: 12 }),
          'a voice line',
        ])
        : null,
      !sealed && song
